...
export class GitManager {
	...
	async getCurrentBranch(): Promise<Result<string, Error>> {
		return await Result.fromAsyncCatching(async () => {
			const result = await this.runCommand(["rev-parse", "--abbrev-ref", "HEAD"]);
			if (!result.ok) {
				throw result.error;
			}
	...
	private async runCommand(args: string[]): Promise<Result<Deno.CommandOutput, Error>> {
	...
	private async runCommandWithErrorContext(args: string[], context: string): Promise<Result<void, Error>> {
